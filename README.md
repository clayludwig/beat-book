...
- **Rate-limit retries.** Tightened the ingest / agent path from 16 × 60s (16 min total) to 8 × 10s (~80s total) so a single stuck request can't blow the full pipeline budget. Extended the same `RateLimitError` retry coverage to cluster labeling and per-topic briefings via a new `_messages_create_with_retry` helper, and dropped `BRIEFING_CONCURRENCY` from 6 → 4 to keep peak in-flight requests under common concurrent-connection caps.
- **Research-agent transient retries.** The opt-in Opus 4.7 stage now retries up to 3 × 10s on `APIStatusError` (5xx), `RateLimitError`, `APIConnectionError`, and `APITimeoutError` before giving up on a turn — a single transient 500 mid-run no longer drops the user back to the pre-research draft. `MAX_TURNS` lowered from 30 → 15 so the model has to finalize sooner; fewer total Opus calls means less cost and a smaller failure surface area.
